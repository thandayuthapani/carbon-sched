...
docker exec -it ${CONTAINER_ID} k6 run --out influxdb=http://${influxdbIP}:${influxdpPort}/myk6db -e SERVICE_URL=${serviceURL} -e FUNCTION_NAME=${serviceName} --console-output ${filename} funcinvoker.js